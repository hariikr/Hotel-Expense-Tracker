...
-- Provides aggregated data for expense tracking insights
...
BEGIN
    ...
    WITH date_series AS (
        SELECT 
            CURRENT_DATE - (days_count - 1) + generate_series(0, days_count - 1) as day
    ),
    daily_incomes AS (
        SELECT 
            i.date,
        ...
        FROM incomes i
        WHERE i.user_id = target_user_id
        AND i.date >= CURRENT_DATE - days_count
        GROUP BY i.date
    ),
    daily_expenses AS (
        SELECT 
        ...
        FROM expenses e
        WHERE e.user_id = target_user_id
        AND e.date >= CURRENT_DATE - days_count
        GROUP BY e.date
    )
    SELECT 
        ds.day::DATE,
...
BEGIN
    ...
    FROM (
        SELECT id FROM incomes WHERE user_id = target_user_id 
        AND EXTRACT(YEAR FROM date) = target_year AND EXTRACT(MONTH FROM date) = target_month
        UNION ALL
        SELECT id FROM expenses WHERE user_id = target_user_id
        AND EXTRACT(YEAR FROM date) = target_year AND EXTRACT(MONTH FROM date) = target_month
    ) as all_transactions;
...
BEGIN
    -- Calculate days
    v_days := end_date - start_date + 1;

    -- Get totals
    SELECT COALESCE(SUM(amount), 0) INTO v_income
    FROM incomes WHERE user_id = target_user_id AND date BETWEEN start_date AND end_date;

    SELECT COALESCE(SUM(amount), 0) INTO v_expense
    FROM expenses WHERE user_id = target_user_id AND date BETWEEN start_date AND end_date;
